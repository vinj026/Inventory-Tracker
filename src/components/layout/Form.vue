...
function handleSubmitForm() {
  ...
  if (!isDuplicate()) {
    const newProduct = { ...formInput, quantity: Number(formInput.quantity) };
    ...
    transactionStore.productTransaction.addTransaction(transactionList)
...
watch(
  ...
  ([newVal1, newVal2]) => {
    console.log("Saved Category updated:", newVal1);
    console.log("Transaction List updated:", newVal2);
  },
  { deep: true }
);
</script>
...
<template>
  <div id="form"
    ...
    <div class="flex justify-between text-black-400 items-center gap-4">
      ...
      <button class="hover:cursor-pointer" @click="ui.formToggle = !ui.formToggle">
        <Minus />
      </button>
    </div>
    ...
    <form class="flex flex-col gap-4" action="">
      <FormField v-for="field in formFields" :key="field.model" :FieldName="field.name" v-model="formInput[field.model]"
        :Type="field.type" :Placeholder="field.placeholder" :HasError="formInputError[field.model].error"
        :ErrorMessage="formInputError[field.model].errorMsg" />

      <div class="relative">
        <FormField FieldName="Category" v-model="formInput.category" Placeholder="e.g. Clothing"
          @focus="inputFocus.isCategoryFocus = true" @blur="inputFocus.isCategoryFocus = false"
          :HasError="formInputError.category.error" :ErrorMessage="formInputError.category.errorMsg" />

        <AutoSuggest v-model="formInput.category" :isInputFocus="inputFocus.isCategoryFocus" />
      </div>

      <div class="flex w-full gap-2 justify-center">
        <FormField FieldName="Quantity" v-model="formInput.quantity" Type="number" Placeholder="e.g. 1000"
          :HasError="formInputError.quantity.error" :ErrorMessage="formInputError.quantity.errorMsg" />
        <div class="flex flex-col gap-1">
          ...
          <div :class="[
            ...
            <DropdownUnit data-testid="dropdown-unit" v-model="formInput.unitType" />
          </div>
          <p v-show="formInputError.unitType.error" class="text-xs font-medium text-orange-900">
    ...
    <Button Name="Submit" @click="handleSubmitForm" class="hover:cursor-pointer" />
  </div>
</template>
